...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing SMSBOMB'
    else
        figlet SMSBOMB
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mWHITEDH4CKER \e[0m"
    else
...
if [ -f .update ];then
    ...
    echo .
    install_deps
    echo This Script Was Made By WHITEDH4CKER > .update
...
do
    ...
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[1;34m RUN SMSBOMB Again..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
